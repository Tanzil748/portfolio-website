    ...
    >
      <div className="xl:container mx-auto py-3 px-2">
        <div className="flex justify-between items-center font-display">
          ...
          {sideMenu && (
            <ul className="flex flex-col justify-center items-center w-full h-screen absolute top-0 left-0 bg-gradient-to-r from-blue-700 via-blue-800 to-gray-900">
              {headerLinks.map(({ name, id }) => (
                <li key={id} className="text-2xl cursor-pointer py-4 font-bold">
                  <Link
                    onClick={() => setSideMenu(!sideMenu)}
                    to={name}
                    smooth
